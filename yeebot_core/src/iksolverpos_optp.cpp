...
double minfuncSumSquaredP(const std::vector<double>& x, std::vector<double>& grad, void* data);

IkSolverPosOPTP::IkSolverPosOPTP(const KDL::Chain& _chain,const KDL::JntArray& _q_min, const KDL::JntArray& _q_max, 
                    Eigen::VectorXi invalid_axis,double _maxtime, double _eps,OptType _type)
...
{   
    ...
    if (chain.getNrOfJoints() < 2) {
        ROS_WARN_THROTTLE(1.0,"IkSolverPosOPTP can only be run for chains of length 2 or more");
        return;
    }
    ...
    case SumSq: 
        ...
        optp.set_min_objective(minfuncSumSquaredP, this);
        break; 
    case L2: 
...
int IkSolverPosOPTP::project(const KDL::JntArray& q_in, const KDL::Frame& m_in, KDL::JntArray& q_out,
    ...
    if (chain.getNrOfJoints() < 2) {
      ROS_ERROR_THROTTLE(1.0,"IkSolverPosOPTP can only be run for chains of length 2 or more");
      return -3;
    }

    if (q_in.data.size() != types.size()) {
      ROS_ERROR_THROTTLE(1.0,"IK seeded with wrong number of joints.  Expected %d but got %d",(int)types.size(), (int)q_in.data.size());
      return -3;
    }
...
void IkSolverPosOPTP::cartSumSquaredErrorP(const std::vector<double>& x, double error[]){
...
double IkSolverPosOPTP::minJoints(const std::vector<double>& x, std::vector<double>& grad)
  {
  ...
  void IkSolverPosOPTP::cartSumSquaredError(const std::vector<double>& x, double error[])
  {
  ...
  void IkSolverPosOPTP::cartL2NormError(const std::vector<double>& x, double error[])
  {
    // Actual function to compute Euclidean distance error.  This uses
    // the KDL Forward Kinematics solver to compute the Cartesian pose
    // of the current joint configuration and compares that to the
  ...
  void IkSolverPosOPTP::cartDQError(const std::vector<double>& x, double error[])
  {
    // Actual function to compute Euclidean distance error.  This uses
    // the KDL Forward Kinematics solver to compute the Cartesian pose
    // of the current joint configuration and compares that to the
  ...
  int IkSolverPosOPTP::CartToJnt(const KDL::JntArray &q_init, const KDL::Frame &p_in, KDL::JntArray &q_out, const KDL::Twist _bounds, const KDL::JntArray& q_desired) {
...
double minfuncSumSquaredP(const std::vector<double>& x, std::vector<double>& grad, void* data) {
    ...
    IkSolverPosOPTP *c = (IkSolverPosOPTP *) data;
